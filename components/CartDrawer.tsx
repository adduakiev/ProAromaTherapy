...
export function CartDrawer({ isOpen, onClose, cart, setCart, exchangeRate, costs, onSaveOrder }: CartDrawerProps) {
  ...
  // ОПТИМІЗАЦІЯ: Розрахунки виконуються лише при зміні кошика чи цін
  const stats = useMemo(() => {
        ...
        <div className="w-screen max-w-md bg-white shadow-2xl flex flex-col border-l border-slate-200">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6 no-scrollbar bg-white">
            <div className="space-y-3">
              {cart.length === 0 ? (
                <div className="text-center py-20 text-slate-300 font-medium italic text-sm">Кошик порожній</div>
              ) : (
                cart.map((item) => (
                  <div key={item.id} className="flex justify-between items-center p-4 bg-[#FDFBF9] rounded-2xl border border-slate-100">
                    ...
                    <button onClick={() => setCart(cart.filter(i => i.id !== item.id))} className="p-2 text-slate-200 hover:text-red-400"><Trash2 className="w-5 h-5" /></button>
                  </div>
                ))
              )}
            </div>
            ...
            {cart.length > 0 && (
              <div className="space-y-4 pt-4 border-t border-slate-50">
                <p className="text-[10px] font-black uppercase text-[#D4A373] tracking-[0.2em] px-1">Клієнт</p>
                <div className="space-y-3">
                  ...
                  <div className="relative flex gap-2">
                    <div className="relative flex-1">
                      ...
                      <input type="tel" placeholder="Телефон" value={customer.phone} onChange={(e) => setCustomer({...customer, phone: e.target.value})} className="w-full pl-11 pr-4 py-3 bg-slate-50 border-none rounded-xl text-sm focus:ring-2 focus:ring-orange-100 outline-none" />
                    </div>
                    <div className="relative flex-1">
                      <Truck className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-300" />
                      <input type="text" placeholder="НП Відділення" value={customer.np} onChange={(e) => setCustomer({...customer, np: e.target.value})} className="w-full pl-11 pr-4 py-3 bg-slate-50 border-none rounded-xl text-sm focus:ring-2 focus:ring-orange-100 outline-none" />
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="p-6 bg-[#F8F3EF] border-t border-slate-200 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-white p-4 rounded-3xl shadow-sm border border-white">
                  ...
                  <p className="text-xl font-black text-slate-800 tracking-tighter">{stats.revenue} грн</p>
                </div>
                <button 
                ...
                >
                  <div className="flex justify-between items-start text-white/70">
                    <p className="text-[9px] font-black uppercase tracking-widest">Прибуток</p>
                    <ChevronRight className="w-3 h-3 group-hover:translate-x-1 transition-transform" />
                  </div>
                  <p className="text-xl font-black text-white tracking-tighter">{Math.round(stats.netProfit)} грн</p>
                </button>
              </div>
              <button 
                onClick={handleSave}
            ...
            <div className="p-8 space-y-6 max-h-[70vh] overflow-y-auto no-scrollbar">
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between bg-slate-50 p-4 rounded-2xl text-xs font-bold text-slate-600">
                    <span>Собівартість рідини</span>
                    <span className="text-slate-900">{stats.totalProductCost.toFixed(1)} грн</span>
                  </div>
                  <div className="flex justify-between bg-slate-50 p-4 rounded-2xl text-xs font-bold text-slate-600">
                    <span>Тара та пляшечки</span>
                    <span className="text-slate-900">{stats.totalTaraCost} грн</span>
                  </div>
                  <div className="flex justify-between bg-slate-50 p-4 rounded-2xl text-xs font-bold text-slate-600">
                    <span>Етикетки ({cart.length} шт)</span>
                    <span className="text-slate-900">{stats.totalLabels} грн</span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-orange-50/30 p-4 rounded-2xl border border-orange-50/50 flex justify-between text-xs font-bold text-orange-900/60">
              ...
              <div className="pt-6 border-t border-slate-100 grid grid-cols-2 gap-6 text-center">
                ...
                <div>
                  ...
                  <p className="text-2xl font-black text-emerald-600 tracking-tighter">{Math.round(stats.netProfit)} <span className="text-xs">грн</span></p>
                </div>
              </div>
            </div>
            <div className="p-4 bg-slate-900 text-center text-[9px] text-white/40 font-black uppercase tracking-[0.2em]">Курс розрахунку: {exchangeRate} грн/€</div>
          </div>
        </div>
      )}
    </div>
